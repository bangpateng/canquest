 ...
 *
 * Cantex DEX TIDAK dipakai untuk pricing lagi — hanya untuk swap.
 * Admin token (untuk key matching) diambil dari Cantex getAllSwapInstruments()
 * supaya konsisten dengan /party/pools (key format = "<id>::<admin>").
 *
 * Cache in-memory 30 detik (pattern sama dengan CantexClient.pricesCache lama).
 * Fail-tolerant: bila scan-proxy gagal → return cache lama (stale better than
 * no data), bila tidak ada cache → return {} (frontend handle price=0).
 */
...
export class CantonPriceService {
  ...
  /** Cache 30 detik — hindari hit scan-proxy + Cantex pools setiap poll. */
  ...
  async getTokenPrices(): Promise<TokenPriceMap> {
    ...
    try {
      // Ambil daftar instrument (id + admin) dari Cantex pools — supaya admin
