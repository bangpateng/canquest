/**
 * List packages on the participant and probe canquest-v2 ACS filter.
...
const packageName = process.env.CANTON_DAML_PACKAGE_NAME?.trim() || 'canquest-v2';
const operator =
...
async function main() {
  ...
  const tpl = `${ref}:CanQuest.Quest.Participation:QuestParticipation`;
