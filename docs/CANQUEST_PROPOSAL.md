...
CanQuest is a production-ready, Canton-native growth platform built to bring **real user activity and recurring fee revenue into the Canton Network** — by helping **other Canton projects acquire verified, active users**.
...
- It increases **real on-chain CC activity** — every user journey involves locking CC, transacting, and paying claim fees.
- It converts **external crypto users into Canton participants** through a familiar quest experience.
- It creates a **fee-based business model tied directly to network usage**, not short-term incentives.
...
- Canton provides the primitives (DAML, parties, CIP-0056); CanQuest turns them into a product people actually use.

The strategic thesis is simple:

> **CanQuest can become the user-facing growth and distribution layer for Canton — while generating usage-based fee revenue from real, verified activity.**
...
### For Canton Network
CanQuest drives **utility-based activity**. Every lock, quest, claim, and transfer moves CC on-chain for a genuine reason. Critically, a holding fee applies while CC is locked — so **every CanQuest lock generates recurring fee revenue for the network** throughout the lock term.
...
### Partner Campaign Hosting
Partners pay to run campaigns on the platform — budget-for-rewards and featured placement.
...
### CIP-0056 Transfers
Claim fees and reward payouts settle through Canton's standard transfer flow, with preapproval-based execution that respects receiver readiness.
...
- users return daily because streaks, points, and leaderboards reward consistency.
...
### Network Revenue
Because locked CC accrues a holding fee, CanQuest user growth translates into recurring fee income for Canton itself.
...
- Canton-native DAML, `LockedAmulet`, and CIP-0056 integration;
- structural Sybil resistance through party-based identity + commitment;
- a fee-generating, retention-driven activity loop;
...
**Mitigation:** Canton-aware backend logic, DAML-enforced fee/reward settlement, preapproval checks, and production configuration. Multiple internal security reviews completed; external DAML audit planned as Milestone 1.
...
The product is already mainnet-deployed, has a clear fee model, and targets users who need real transaction utility. It helps Canton expand the ecosystem beyond institutional infrastructure by supporting a user-facing product that drives **activity, volume, and recurring network revenue**.
...
- It turns CC locks into **recurring fee revenue for the network**.
